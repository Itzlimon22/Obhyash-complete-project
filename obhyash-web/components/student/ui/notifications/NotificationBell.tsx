...
const NotificationBell: React.FC<NotificationBellProps> = ({
  ...
  return (
    <button
      onClick={handleClick}
      className={`relative p-2.5 rounded-full transition-all duration-300 group flex items-center justify-center border ${
        isOpen
    ...
    >
      <Bell
        className={`w-5 h-5 transition-all duration-300 ${isOpen ? 'scale-110' : 'group-hover:scale-110 group-active:scale-95'}`}
        strokeWidth={2}
      />
