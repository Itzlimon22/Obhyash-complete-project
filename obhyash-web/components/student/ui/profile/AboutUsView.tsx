              ...
              {(Object.keys(POLICY_CONTENT) as PolicyKey[]).map((key) => (
                <button
                  ...
                  className={cn(
                    ...
                    activePolicy === key
                      ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-200 dark:shadow-none translate-x-1'
                ...
                >
                  <div
                    className={cn(
                      ...
                      activePolicy === key
                        ? 'bg-white/20'
                        : 'bg-neutral-100 dark:bg-neutral-800 group-hover:bg-emerald-50 dark:group-hover:bg-emerald-900/30',
                    )}
                  >
              ...
              <div className="grid gap-4 md:gap-6">
                {(content.sections as Section[]).map((section, idx) => (
                  <motion.div
                    ...
                    className="group"
                  >
                    <div className="bg-white dark:bg-neutral-900 rounded-3xl border border-neutral-100 dark:border-neutral-800 p-5 md:p-8 hover:border-emerald-200 dark:hover:border-emerald-900/50 hover:shadow-lg hover:shadow-emerald-500/5 transition-all duration-300">
                      <div className="flex flex-col md:flex-row items-start gap-4 md:gap-6">
                        <div className="flex-none flex items-center justify-center w-10 h-10 md:w-12 md:h-12 rounded-2xl bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 font-black text-lg shadow-inner group-hover:scale-110 transition-transform">
                          {section.id || idx + 1}
                        </div>
                        <div className="flex-1 space-y-3 md:space-y-4">
                          <h3 className="text-lg md:text-xl font-black text-neutral-900 dark:text-white group-hover:text-emerald-600 transition-colors">
                            {section.title}
                          </h3>
                          ...
                          {section.warning && (
                            <div className="mt-4 p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-2xl border border-emerald-100 dark:border-emerald-900/50 text-emerald-700 dark:text-emerald-400 text-sm font-bold flex items-center gap-2">
