  ...
  return (
    <div
      className={`
        ...
        ${
          isCurrent
            ? `bg-white dark:bg-neutral-900 border-2 ${getThemeClasses()} shadow-md scale-[1.01]`
            : `bg-white dark:bg-neutral-900 border ${getThemeClasses()} shadow-sm hover:shadow-lg hover:-tranneutral-y-1`
        }
      `}
    >
      {plan.isPopular && (
      ...
      {/* Compact Header */}
      <div
        className={`p-4 border-b border-neutral-100 dark:border-neutral-800/50 ${getHeaderBg()}`}
      >
        <h3
        ...
        <div className="flex items-baseline gap-0.5">
          ...
          {!isFree && (
            <span className="text-neutral-500 dark:text-neutral-400 text-[10px] font-bold uppercase">
              {plan.billingCycle}
            </span>
          )}
        </div>
      ...
      <div className="p-4 flex flex-col flex-1">
        <ul className="space-y-2 mb-5 flex-1">
          {plan.features.map((feature, idx) => (
            <li
              key={idx}
              className="flex items-start gap-2 text-xs font-medium text-neutral-600 dark:text-neutral-300 leading-snug"
            >
              <div
                className={`mt-0.5 w-3.5 h-3.5 rounded-full flex items-center justify-center shrink-0 ${plan.colorTheme === 'neutral' ? 'bg-neutral-100 dark:bg-neutral-800 text-neutral-500' : 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600'}`}
              >
                <svg
                  className="w-2 h-2"
                  fill="none"
        ...
        <button
          ...
          className={`
                ...
                ${
                  isCurrent
                    ? 'bg-neutral-100 dark:bg-neutral-800 text-neutral-400 cursor-default border border-neutral-200 dark:border-neutral-700'
                    : `text-white shadow-md shadow-${plan.colorTheme}-500/20 ${plan.colorTheme === 'emerald' ? 'bg-emerald-600 hover:bg-emerald-700' : plan.colorTheme === 'indigo' ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-neutral-800 hover:bg-neutral-900 dark:bg-white dark:text-neutral-900 dark:hover:bg-neutral-200'}`
                }
            `}
        >
