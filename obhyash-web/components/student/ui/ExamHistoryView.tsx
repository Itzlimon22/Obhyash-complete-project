          ...
          <div className="lg:col-span-4 space-y-4">
            <div className="bg-white dark:bg-neutral-900/50 backdrop-blur-sm p-5 rounded-[2rem] border border-neutral-200 dark:border-neutral-800 shadow-sm sticky top-20">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="font-bold text-neutral-800 dark:text-white">সার্চ ও ফিল্টার</h3>
              </div>

              <div className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-neutral-500 uppercase ml-1">বিষয় অনুযায়ী খুঁজুন</label>
                  <input
                    type="text"
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-neutral-500 uppercase ml-1">তারিখ অনুযায়ী</label>
                  <input
                    type="date"
                    value={filterDate}
                    onChange={(e) => {
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            {displayedHistory.length === 0 ? (
              <div className="text-center py-20 bg-white dark:bg-neutral-900/50 rounded-[2.5rem] border border-neutral-200 dark:border-neutral-800 border-dashed">
                ...
                <h3 className="text-xl font-black text-neutral-900 dark:text-white">কোন ফলাফল পাওয়া যায়নি</h3>
                <p className="text-neutral-500 dark:text-neutral-400 mt-2">ফিল্টার অনুযায়ী কোনো তথ্য নেই।</p>
              </div>
            ) : (
              <div className="space-y-4">
                {displayedHistory.map((item) => {
                  const isPending = item.submissionType === 'script' && item.status === 'pending';
                  const isRejected = item.submissionType === 'script' && item.status === 'rejected';
                        ...
                        <div className="flex-1 space-y-3">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {item.submissionType === 'script' && (
                              <span className="px-2 py-0.5 rounded-lg bg-neutral-100 dark:bg-neutral-800 text-[10px] font-bold text-neutral-500 uppercase border border-neutral-200 dark:border-neutral-700">SCRIPT</span>
                            )}
                            {isPending && <span className="px-2 py-0.5 rounded-lg bg-amber-100 dark:bg-amber-900/30 text-[10px] font-bold text-amber-600 dark:text-amber-400 uppercase">PENDING</span>}
                            {isRejected && <span className="px-2 py-0.5 rounded-lg bg-red-100 dark:bg-red-900/30 text-[10px] font-bold text-red-600 dark:text-red-400 uppercase">REJECTED</span>}
                          </div>
                        ...
                        <div className="w-full md:w-auto flex items-center justify-between md:justify-end gap-6 shrink-0 border-t md:border-t-0 pt-4 md:pt-0">
                          {isPending ? (
                            <div className="text-right flex-1 md:flex-none">
                              <p className="text-[10px] font-bold text-neutral-400 uppercase mb-1">প্রক্রিয়াধীন</p>
                            ...
                            <div className="flex items-center gap-6">
                              <div className="text-center">
                                <p className="text-[10px] font-bold text-neutral-400 uppercase mb-0.5">স্কোর</p>
                                <div className="text-xl font-black text-neutral-900 dark:text-white leading-none">
                                  {item.score.toFixed(0)}
                                  <span className="text-xs text-neutral-400 font-bold ml-0.5">/ {item.totalMarks}</span>
                                </div>
                              </div>
                              
                              <div className={`relative w-12 h-12 flex items-center justify-center rounded-full border-4 ${
                                percentage >= 80 ? 'border-emerald-500/20' : percentage >= 50 ? 'border-amber-500/20' : 'border-red-500/20'
                              }`}>
                                <div className={`absolute inset-0 rounded-full border-4 border-t-transparent -rotate-45 ${
                                  percentage >= 80 ? 'border-emerald-500' : percentage >= 50 ? 'border-amber-500' : 'border-red-500'
                ...
                {hasMore && (
                  <div className="flex justify-center mt-8">
                    ...
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-4 h-4 text-rose-500">
