...
const QuestionPalette: React.FC<QuestionPaletteProps> = ({ 
  ...
  const getStatusClass = (id: number) => {
    ...
    if (isFlagged) return 'bg-amber-400 text-white border-amber-500 ring-2 ring-amber-100 dark:ring-amber-900/50';
    if (isAnswered) return 'bg-emerald-500 text-white border-emerald-600';
    return 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-300 dark:border-slate-600 hover:border-indigo-400 dark:hover:border-indigo-400 hover:bg-slate-50 dark:hover:bg-slate-700';
  };
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto pr-1">
        <div className="grid grid-cols-4 gap-2.5">
          {questions.map((q, idx) => (
            <button
              key={q.id}
              onClick={() => onQuestionClick(typeof q.id === 'string' ? parseInt(q.id) : q.id)}
              className={`
      ...
      <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800 space-y-3">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-slate-700 dark:text-slate-300">উত্তর দেওয়া হয়েছে</span>
        </div>
        <div className="flex items-center gap-3">
          <div className="w-5 h-5 rounded bg-amber-400 border border-amber-500"></div>
          <span className="text-sm text-slate-700 dark:text-slate-300">রিভিউয়ের জন্য মার্ক করা</span>
        </div>
        <div className="flex items-center gap-3">
          <div className="w-5 h-5 rounded bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600"></div>
          <span className="text-sm text-slate-700 dark:text-slate-300">উত্তর দেওয়া হয়নি</span>
        </div>
      </div>
    </div>
  );
