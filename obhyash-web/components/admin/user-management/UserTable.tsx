  ...
  return (
    <div className="space-y-4">
      ...
      {(viewStyle === 'responsive' || viewStyle === 'card') && (
        <div
          className={`space-y-3 ${viewStyle === 'responsive' ? 'lg:hidden' : ''}`}
        >
          {users.map((user) => (
            <div
              key={user.id}
      ...
      {(viewStyle === 'responsive' || viewStyle === 'table') && (
        <div
          className={`bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800 shadow-sm overflow-hidden text-center ${viewStyle === 'responsive' ? 'hidden lg:block' : ''}`}
        >
