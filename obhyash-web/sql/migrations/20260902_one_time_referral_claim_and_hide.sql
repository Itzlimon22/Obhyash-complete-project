-- ==============================================================================
-- Migration: One-Time Lifetime Referral Claim & Auto-Hide for Registered Users
-- Description:
--   1. Ensures referral claiming is strictly 1-TIME per user account (lifetime).
--   2. If a user registers with a referral code OR claims one later, it hides the claim section.
--   3. The user's OWN referral code can still be shared with unlimited other users.
-- ==============================================================================

-- 0. Clean up any previous overloaded versions to avoid "Could not choose the best candidate function"
DROP FUNCTION IF EXISTS public.redeem_referral_by_code(text, uuid);
DROP FUNCTION IF EXISTS public.redeem_referral_by_code(text, uuid, text);
DROP FUNCTION IF EXISTS public.redeem_referral_by_code(text, uuid, text, text);
DROP FUNCTION IF EXISTS public.redeem_referral_tx(uuid, uuid);
DROP FUNCTION IF EXISTS public.redeem_referral_tx(uuid, uuid, text);
DROP FUNCTION IF EXISTS public.redeem_referral_tx(uuid, uuid, text, text);
DROP FUNCTION IF EXISTS public.check_referral_eligibility(uuid);

-- 0.1 Ensure referral_history columns exist
ALTER TABLE IF EXISTS public.referral_history ADD COLUMN IF NOT EXISTS device_id text;
ALTER TABLE IF EXISTS public.referral_history ADD COLUMN IF NOT EXISTS ip_address text;
ALTER TABLE IF EXISTS public.referral_history ADD COLUMN IF NOT EXISTS admin_status text DEFAULT 'Approved';
ALTER TABLE IF EXISTS public.referral_history ADD COLUMN IF NOT EXISTS reward_given boolean DEFAULT true;

-- 0.2 Ensure referral_claim_attempts table exists for anti-brute force rate limiting
...
-- Enable RLS on referral_claim_attempts
ALTER TABLE public.referral_claim_attempts ENABLE ROW LEVEL SECURITY;

DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'referral_claim_attempts' AND policyname = 'Users can view own claim attempts'
    ) THEN
        CREATE POLICY "Users can view own claim attempts" ON public.referral_claim_attempts
            FOR SELECT TO authenticated USING (auth.uid() = user_id);
    END IF;
END $$;

GRANT ALL ON TABLE public.referral_claim_attempts TO service_role, postgres;
GRANT SELECT, INSERT, UPDATE ON TABLE public.referral_claim_attempts TO authenticated;

-- 1. Updated redeem_referral_tx function
...
BEGIN
    ...
    -- 3. LIFETIME ONE-TIME CLAIM CHECK: Check if redeemer user has ALREADY claimed any referral code
    IF EXISTS (
        SELECT 1 FROM public.referral_history
        WHERE redeemed_by = p_redeemer_id
    ) THEN
        RAISE EXCEPTION 'তুমি ইতিমধ্যে একটি রেফারেল কোড ক্লেইম করেছ। এটি শুধুমাত্র একবার ব্যবহারযোগ্য।';
    END IF;
    ...
    BEGIN
        ...
        ELSE
            -- Normal notification for code owner (+7 days Pro reward)
            INSERT INTO public.notifications (
                user_id,
                title,
                message,
    ...
    -- 6. Insert notification for redeemer (new user +15 days Pro)
    INSERT INTO public.notifications (
        user_id,
        title,
        message,
...
-- 2. Updated redeem_referral_by_code function
...
BEGIN
    ...
    -- LIFETIME ONE-TIME CLAIM CHECK: Check if user already claimed any referral
    IF EXISTS (
        SELECT 1 FROM public.referral_history
...
-- 3. Helper RPC to check if user has claimed referral & eligibility
...
BEGIN
    SELECT EXISTS (
        SELECT 1 FROM public.referral_history WHERE redeemed_by = p_user_id
    ) INTO v_has_used;

    SELECT * INTO v_attempt FROM public.referral_claim_attempts WHERE user_id = p_user_id;
    IF v_attempt.user_id IS NOT NULL THEN
        IF v_attempt.lockout_until IS NOT NULL AND v_attempt.lockout_until > v_now THEN
            v_lock_sec := EXTRACT(EPOCH FROM (v_attempt.lockout_until - v_now))::int;
            v_rem_att := 0;
        ELSE
            v_rem_att := 3 - COALESCE(v_attempt.failed_attempts, 0);
        END IF;
    END IF;

    RETURN json_build_object(
        'has_used_referral', v_has_used,
        'remaining_attempts', v_rem_att,
        'lock_seconds', v_lock_sec
    );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

GRANT EXECUTE ON FUNCTION public.redeem_referral_tx(UUID, UUID, TEXT, TEXT) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.redeem_referral_by_code(TEXT, UUID, TEXT, TEXT) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.check_referral_eligibility(UUID) TO authenticated, service_role;
