...
/**
 * Interface for the ReviewTable Props to resolve TypeScript errors.
 * data: The array of normalized question objects.
 * onEdit: Callback to open the edit modal for a specific question.
 * onDelete: Callback to remove a question from the queue.
 */
  ...
  if (data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-40 border-2 border-dashed rounded-lg bg-gray-50">
        <p className="text-muted-foreground">No questions to preview. Upload a file to begin.</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-md border h-[400px] overflow-auto shadow-inner bg-white">
      <Table>
        ...
        <TableBody>
          {data.map((q, i) => (
            <TableRow key={i} className="hover:bg-slate-50 transition-colors border-b">
