      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto text-black bg-white">
        ...
        <div className="grid gap-6 py-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-slate-50 rounded-lg border">
            ...
            <div className="space-y-2">
              <Label className="text-xs text-slate-500 uppercase">Chapter</Label>
              <Input
            ...
            <div className="space-y-2">
               <Label className="text-xs text-slate-500 uppercase">Difficulty</Label>
               <Select 
             ...
             <div className="space-y-2">
              <Label className="text-xs text-slate-500 uppercase">Exam Type</Label>
              <Input
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <div className="p-4 bg-slate-50 border rounded-md min-h-[60px]">
                <p className="text-[10px] text-slate-400 mb-1 uppercase tracking-wider">Preview</p>
                <div className="text-slate-800">
                  <MathRenderer text={formData.question || 'Start typing to see preview...'} />
                </div>
              </div>
            </div>
          ...
          {/* ✅ 3. Options with Latex Preview */}
          <div className="space-y-2">
             <Label>Options</Label>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {formData.options.map((opt, i) => (
                <div key={opt.id || i} className="p-3 border rounded-lg bg-slate-50/50 space-y-2">
                  <div className="flex justify-between items-center">
                    <Label className="text-xs font-bold text-slate-500 uppercase">
                      Option {opt.id}
                      {opt.isCorrect && <span className="ml-2 text-emerald-600">(Correct)</span>}
                    </Label>
                  </div>
                  
                  <div className="flex gap-2">
                    <div className={`flex items-center justify-center w-8 h-10 rounded text-sm font-bold ${opt.isCorrect ? 'bg-emerald-100 text-emerald-700' : 'bg-white border text-slate-500'}`}>
                      {opt.id}
                    </div>
                    <Input
                      value={opt.text}
                      onChange={(e) => updateOptionText(i, e.target.value)}
                      className={opt.isCorrect ? 'border-emerald-500 ring-1 ring-emerald-500' : 'bg-white'}
          ...
          <div className="space-y-2 bg-emerald-50 p-4 rounded-lg border border-emerald-100">
            <Label className="text-emerald-700 font-bold">Set Correct Answer</Label>
              ...
              <SelectContent>
                {formData.options.map((opt) => (
                  <SelectItem key={opt.id} value={opt.id}>
                    <span className="font-bold mr-2 text-slate-500">{opt.id.toUpperCase()}</span>
          ...
          <div className="space-y-2">
            ...
            {formData.explanation && (
              <div className="p-3 bg-slate-50 border rounded-md text-sm">
                <p className="text-[10px] text-slate-400 mb-1 uppercase">Solution Preview</p>
